...
% INPUTS
nSamp = 1e6;  % No. of Samples
nBars = 41; % No. of bars to show on histograms

Latitude = 33.5;    % Latitude of the solar panel (determines the zenith angle @ noon)
...
if (Latitude > 23.4)
    MaxAngle = (90-Latitude + 23.4).*pi./180;  % Maximum angle of the sun relative to the horizon [rad]
else
...
%% Plotting
thetad = (theta).*180./pi;   % Angle distribution in deg [º]
...
if (PowerPlots)
    figure(3)
    subplot(1,2,1)
    plot(thetad,PowerOut,'k.','MarkerSize',0.75)
    title({'Power Output PDF';[num2str(nSamp) ' Cases']},'fontsize',12);
    xlabel('Degrees (º)','fontsize',12)
    ylabel('Power (Wm^-^2)','fontsize',12);
    grid on
...
if (ProbabilityPlots)
    ...
    plot(bins,POcdf,'k','LineWidth',1.5);
